...
def test(G1, test_dataset):
    ...
    for n in tqdm(range(len(test_dataset)), desc="Testing"):
        ...
        with torch.no_grad():
            ...
            in_img = torch.cat([img, gt_shadow], dim=1)
            
            # Get shadow-free output
            shadow_removal_image = G1.test_set(in_img.to(device))
        ...
        grid = make_grid(torch.cat([unnormalize(img), 
                                    unnormalize(gt), 
                                    unnormalize(shadow_removal_image)], dim=0))
        
        # Save grid image
        save_image(grid, f'./test_result/shadow_removal_grid_images/{filename}')

        # Save individual shadow-free image
...
def main(parser):
    ...
    if parser.load is not None:
        checkpoint_path = f'./checkpoints/DPGLO_SRNet_G1_{parser.load}.pth'
